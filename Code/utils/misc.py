...
def check_resume(opt, resume_iter):
    ...
    # 1. 注释掉或删除与 resume_state 相关的条件分支和代码

    # 2. 获取所有网络的名称
    networks = [key for key in opt.keys() if key.startswith('network_')]

    # 3. 初始化一个布尔变量 flag_pretrain，用于标记是否存在预训练模型的路径
    flag_pretrain = False

    # 4. 遍历每个网络的名称，检查是否存在对应的预训练模型的路径
    for network in networks:
        if opt['path'].get(f'pretrain_{network}') is not None:
            flag_pretrain = True

    # 5. 如果存在预训练模型的路径，则记录一个警告信息
    if flag_pretrain:
    ...
    for network in networks:
        ...
        if opt['path'].get('ignore_resume_networks') is None or (
                basename not in opt['path']['ignore_resume_networks']):
            # # 3. 修改为你自己的预训练模型路径
            opt['path'][name] = '/home/ls/ls/RetinexMamba-main/experiments/RetinexMamba_LOL_v2_real/models/net_g_93800.pth'  # 修改为你的预训练模型路径
            logger.info(f"Set {name} to {opt['path'][name]}")
